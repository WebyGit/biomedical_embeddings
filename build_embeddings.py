...
fasttext.skipgram(DATA_FILE, os.path.join(MODELS_DIR, 'fasttext_skipgram'),
                  silent=0, thread=WORKERS, dim=DIMENSIONS)
print('Done with fasttext skipgram')

print('Building fasttext cbow')
fasttext.cbow(DATA_FILE, os.path.join(MODELS_DIR, 'fasttext_cbow'),
              silent=0, thread=WORKERS, dim=DIMENSIONS)
print('Done with fasttext cbow')

print('Building word2vec skipgram')
Word2Vec(LineSentence(DATA_FILE), size=300, sg=1, workers=WORKERS) \
    .save(os.path.join(MODELS_DIR, 'word2vec_skipgram'))
print('Done with word2vec skipgram')

print('Building word2vec cbow')
Word2Vec(LineSentence(DATA_FILE), size=300, sg=0, workers=WORKERS) \
    .save(os.path.join(MODELS_DIR, 'word2vec_cbow'))
